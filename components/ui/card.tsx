...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) rounded-lg border-2 border-[#2d3835] bg-[#181818] py-(--card-spacing) text-sm text-card-foreground shadow-[4px_4px_0px_0px_#0d3b2e] [--card-spacing:--spacing(6)] has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(4)]",
        className
      )}
      {...props}
    />
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-sm text-[#a7a7a7]", className)}
      {...props}
    />
  )
}
